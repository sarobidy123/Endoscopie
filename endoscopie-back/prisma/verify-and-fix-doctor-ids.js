...
async function verifyAndFixDoctorIds() {
  try {
    ...
    if (doctors.length > 0) {
      ...
      doctors.forEach((d, i) => {
        console.log(`   ${i + 1}. ${d.prenom} ${d.nom} (ID: ${d.id.substring(0, 8)}..., ${d.specialite})`);
      });
      console.log();
    }
    ...
    invalidPrescriptions.forEach((p, i) => {
      console.log(`   ${i + 1}. Prescription ${p.id.substring(0, 8)}...`);
      console.log(`      - Invalid medecinId: ${p.medecinId.substring(0, 8)}...`);
      console.log(`      - Patient: ${p.patientId.substring(0, 8)}...`);
    });
    ...
    const updatedPrescriptions = await prisma.prescription.findMany();
